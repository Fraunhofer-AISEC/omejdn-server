...
class OAuth2Test < Test::Unit::TestCase
  ...
  def setup
    ...
    @pubclient = Client.find_by_id 'publicClient'
  ...
  def request_client_credentials(client, alg, key, certificate, query_additions='', should_work=true)
    query = 'grant_type=client_credentials'+
            ...
            '&client_assertion='+get_private_key_jwt(client,alg,key,certificate)+
  ...
  def test_client_credentials_with_resources
    ...
    response = request_client_credentials @pubclient, "ES256", @priv_key_ec256, @certificate_ec256, resources, false
    resources = '&resource=http://example.org'
    response = request_client_credentials @pubclient, "ES256", @priv_key_ec256, @certificate_ec256, resources
    at = extract_access_token response
    ...
    assert_equal at['nbf']+response["expires_in"], at['exp']
    assert       at['jti']
    assert_equal @pubclient.client_id, at['client_id']
    assert_equal at['client_id'], at['sub']
  end
  ...
  def request_authorization(user, client, query_additions='', should_work=true, scopes = ['omejdn:write'])
    ...
    '&redirect_uri='+client.metadata['redirect_uris']+
    '&client_assertion_type=urn:ietf:params:oauth:client-assertion-type:jwt-bearer'+
    '&client_assertion='+get_private_key_jwt(client,"ES256", @priv_key_ec256, @certificate_ec256)
    post ('/token?'+query), {}, {}
    good_so_far &= last_response.ok?
    # p last_response
  ...
  def test_authorization_flow_with_bad_resources
    resources = '&resource=a&resource=b'
    response = request_authorization TestSetup.users[0], @pubclient, resources, false
  end

  def test_authorization_flow_with_resources
    resources = '&resource=http://example.org'
    response = request_authorization TestSetup.users[0], @pubclient, resources
    at = extract_access_token response
  ...
  def test_authorization_flow_with_request_uri
    ...
    post '/par', {
      ...
      'request'=>jwt,
      'client_assertion_type' => 'urn:ietf:params:oauth:client-assertion-type:jwt-bearer',
      'client_assertion' => get_private_key_jwt(@client,"RS256", @priv_key_rsa, @certificate_rsa)
      }, {}
    # p last_response
